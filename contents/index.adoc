...
[source,diff]
----
include::{samplescodedir}/step-2/build.gradle.kts[tags=configure-dependencies]
----
...
----
<1> Creates a new `dokkaJar` task of type `Jar`. You don't need to qualify `Jar` because its package is auto imported by Gradle.
...
TIP: You can add an `import` line to the build script for the `DokkaTask` task type. Just be aware that it must go at the top of the build script, before everything else.
